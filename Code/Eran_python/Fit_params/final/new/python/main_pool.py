...
from misc import idcs2state, convert_params, get_a_opp, replay_2moves, state2idcs, get_new_state, tic, toc
...
def main(params):
    ...
    Q1, Q2 = replay_2moves(Q1, Q2, T2, world2, p['evm_threshold'], p['beta2'], p['beta1'], p['alpha2'], p['alpha1'])
    ...
    Q1, Q2 = replay_2moves(Q1, Q2, new_T2, world2, p['evm_threshold'], p['beta2'], p['beta1'], p['alpha2'], p['alpha1'])
...
def get_hypers(pars):
    ...
    for j in range(len(mean)):
        if j in [3, 4, 6, 7, 8, 9, 12, 13, 15]:
            while True:
                this_param = np.random.normal(mean[j], var)
                if this_param < 0 or this_param > 1:
                    pass
                else:
                    break
        elif j in [0, 1, 2, 14]:
...
N = 20
num_proc = 3
hypers   = [sample_hypers() for i in range(num_proc)]

with Pool(num_proc) as p:
    t1 = tic()
    tout = p.map(main, hypers)
    t2 = toc(t1)
    out  = p.map(dist, tout)
    idx  = np.argmin(out)
    eps_t  = out[idx]
...
print('Done in %.4f'%t2)
variance = np.logspace(np.log10(1), np.log10(0.02), num=N)

with Pool(num_proc) as p:
    for n in range(N):
        var = variance[n]
        while True:
            hypers = p.map(get_hypers, zip([Q_t]*num_proc, [var]*num_proc))
            t1 = tic()
            tout = p.map(main, hypers)
            t2 = toc(t1)
            print('Done in %.4f'%t2)
