...
def step(info_in,sim_pool=None):
    ...
    while True:
        ...
        if np.any(rho <= tol[t]):
            print('Found rho < tol!', flush=True)
            idcs = np.argwhere(rho <= tol[t])[0][0]
            break
    print(rho[idcs], trial_t[idcs], flush=True)
...
class ABC_class(object):
        ...
        def sample_loop(self,t):
                ...
                print('Sampling at iteration %u'%t)
        ...
        def classstep(self,info_in):
            ...
            while True:
                if t ==0: #draw from prior
                    ...
                    repeat = True
                    while repeat:
                        this_t = np.atleast_1d(scipy.stats.multivariate_normal.rvs(mean= t_old,cov=covariance,size=1))
                        cond1 = this_t[3] < 0 or this_t[3] > 1
                        cond2 = this_t[4] < 0 or this_t[4] > 1
                        cond3 = this_t[6] < 0 or this_t[6] > 1
                        cond4 = this_t[7] < 0 or this_t[7] > 1
                        cond5 = this_t[8] < 0 or this_t[8] > 1
                        cond6 = this_t[9] < 0 or this_t[9] > 1
                        cond7 = this_t[12] < 0 or this_t[12] > 1
                        cond8 = this_t[13] < 0 or this_t[13] > 1
