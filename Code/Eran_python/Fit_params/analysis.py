...
def analyse_1move(data_path, world, state_arr):
    ...
    for f in range(len(all_files_sorted)):
        ...
        Q    = data['Q_history']
        evm  = np.atleast_2d(data['replay_gain']) * 1/8
        T    = data['T']
...
def analyse_2moves(data_path, world, state_arr):
    ...
    for d in range(0, 2):
        ...
        for f in range(len(all_files_sorted)):
            ...
            Q   = data['Q%u_history'%(d+1)]
            evm = data['replay_gain%u'%(d+1)] * 1/8
            T   = data['T']
            ...
            replay_backups_all = np.atleast_2d(data['replay_backups'])
            replay_backups = replay_backups_all[np.where(replay_backups_all[:, 4] == d)]
            if replay_backups.shape[0] > 0:
                c = 0
                for i in range(replay_backups.shape[0]):
                    ...
                    this_Q  = Q[c].reshape(8, -1)
                    
                    if this_Q[sp, ap] <= Q_true[sp, ap]:
                        pos_TD[(sp*num_acts + ap)] += 1
                    else:
                        neg_TD[(sp*num_acts + ap)] += 1
                    ...
                    #     else:
                    #         if d == 0:
                    #             tmp_entropy -= T[sp, ap, j]*np.log2(T[sp, ap//4, j]*[sp, ap//4, j])
                    # H[(sp*4 + ap)].append(tmp_entropy)
