...
def get_new_state(si, a, world, states):
    ...
    a      – chosen action
    states – spatial arrangement of states
    '''
...
def replay_1move(Q2, T2, world, beta, alpha2, evm_thresh):
    '''
    Generate replay experiences for 1-move trials and execute the ones above the evm threshold
    
    Parameters:
    Q2         – Q values for 1-move trials
    T2         – state-transition matrix
    world      – image-reward associations
    beta       – inverse temperature parameter
    alpha2     – replay learning rate
    evm_thresh – evm threshold
    '''
...
def replay_2moves(Q1, Q2, T2, world, evm_thresh, beta2, beta1, alpha2, alpha1):
    ...
    for sr in range(8):
        for ar in range(4):
            this_action_probas = T2[sr, ar, :].copy()
            s1r  = np.argmax(this_action_probas)
