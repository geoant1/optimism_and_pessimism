...
def plot_q_values_maintenance(av_rew, Q_all, Q_true, title: str, save_path: str):
    ...
    add      = np.linspace(-0.24, 0.24, num_acts)
    ...
    if num_acts == 4:
        ...
        c = ["#"+''.join([np.random.choice('0123456789ABCDEF') for j in range(6)]) for i in range(num_acts)]
    ...
    for s in range(8):
        for a in range(4):
            ...
            ax.bar(this_x, Q_av[s, a], width=w, yerr=np.sqrt(err[s*4 + a]), capsize=3, color=c[a], alpha=0.5)
            ax.hlines(Q_true[s, a], this_x-w/2, this_x+w/2)
            ax.annotate('%s'%Q_true[s, a], (this_x-w/2 +0.02, 0.03), fontsize=7, color='k')
...
def plot_replay_frequency(pos_TD, neg_TD, Q_true, title: str, save_path: str):
    ...
    x_pos    = np.arange(0, 8)
    num_acts = int(Q_true.shape[1]/8)
    x_add    = np.linspace(-0.24, 0.24, num_acts)
    ...
    for i in range(len(idcs)):
        ax.annotate('%.1f'%Q_true[i], (idcs[i]-width/2 +0.02, 0.03), fontsize=7, color='k')
