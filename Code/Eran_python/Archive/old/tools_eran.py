...
from misc_eran import state2idcs, get_new_state
import os
import shutil
...
def plot_replay_frequency(pos_TD, neg_TD, Q_true, title: str, save_path: str):
    ...
    x_pos   = np.arange(0, 8)
    x_add   = [-0.24, -0.08, 0.08, 0.24]
    x_ticks = ['N','S','W','E']*8

    width = 0.14
    idcs = [i+x_add[j] for i in range(8) for j in range(4)]
