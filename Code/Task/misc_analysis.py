...
def policy_improve_2moves(q1_before, q1_after, q2_before, q2_after, Q1_true, Q2_true, s, beta1, beta2, biases, world, idcs, mode='value', benefit='objective'):
    ...
    #     probs   = policy_choose(tmp, beta1)
    #     q_vals1b[a] = np.nansum(tmp*probs)
    # q_vals2b = q2_before[s, :].copy()

    # probs_before1 = policy_choose_moves(q_vals1b, q_vals2b, beta1, beta2)
    ...
    # q_vals2a = q2_before[s, :].copy()

    # probs_after1 = policy_choose_moves(q_vals1a, q_vals2a, beta1, beta2)
    ...
    #         for a2 in a2_opt:
    #             probs_before2 = policy_choose_moves(q1_before[(a1*4):(a1*4+4)], q2_before[s1, :], beta1, beta2)
    #             probs_after2  = policy_choose_moves(q1_after[(a1*4):(a1*4+4)], q2_before[s1, :], beta1, beta2)
    ...
    #             s1            = idcs2state(s1i, idcs)
    #             probs_before2 = policy_choose_moves(q1_before[(a*4):(a*4+4)], q2_before[s1, :], beta1, beta2)
    #             tmpb.append(np.nansum(probs_before2*Q1_true[s, (a*4):(a*4+4)]))
            
    #             probs_after2 = policy_choose_moves(q1_after[(a*4):(a*4+4)], q2_before[s1, :], beta1, beta2)
        ...
        for j in range(this_range):
            ...
            for f in range(len(all_files_sorted)):
                ...
                if len(move) == 2:
                    ...
                    # first_single_move_optimal  = a1 in get_optimal_move(s1, Q2_true)
                    second_single_move_optimal = a1 in get_optimal_move(s1, Q1_true)%4
                    first_single_move_optimal  = a2 in get_optimal_move(s1, Q1_true)//4
                    ...
                    if a1 in [0, 1]:
                        ...
                        ja1 = [a1, (1-a1)] # up (0) / down (1) in first move
                    if a2 in [0, 1]:
                        ja  = [a1*4 + a2, a1*4+ (1-a2)] # first move + up (0) /down (1)
                        ja2 = [a2, (1-a2)] # up (0) /down (1) in second move
                    
                    if replay_backups.shape[0] > 1:
                        
                        # replays of paired move
                        ...
                        # replays of first move
                        s_cond = replay_backups[:, 0]  == s1
                        a_cond = (replay_backups[:, 1] == ja1[0])
                        p_cond = (replay_backups[:, 4] == 1)
                        idcs_first_single = np.argwhere(s_cond & a_cond & p_cond).flatten()
                        
                        if len(ja1) > 1:
                            a_cond_similar      = (replay_backups[:, 1] == ja1[1])
                            idcs_first_single_similar = np.argwhere(s_cond & a_cond_similar & p_cond).flatten()
                        else:
                            idcs_first_single_similar = []
                            
                        # replays of second move
                        # NB replays of first moves are not included since optimal first moves in 
                        ...
                        p_cond = (replay_backups[:, 4] == 1)
                        idcs_second_single = np.argwhere(s_cond & a_cond & p_cond).flatten()
                        
                        if len(ja2) > 1:
                            a_cond_similar      = (replay_backups[:, 1] == ja2[1])
                            idcs_second_single_similar = np.argwhere(s_cond & a_cond_similar & p_cond).flatten()
                        else:
                            idcs_second_single_similar = []
                            
                        num_paired                = len(idcs_paired)
                        num_paired_similar        = len(idcs_paired_similar)
                        num_first_single          = len(idcs_first_single)
                        num_first_single_similar  = len(idcs_first_single_similar)
                        num_second_single         = len(idcs_second_single)
                        num_second_single_similar = len(idcs_second_single_similar)
                        
                        # If the sequence was optimal
                        if paired_move_optimal:

                            tmp_opt += num_paired + num_paired_similar
                            ...
                            tmp_subopt += num_paired + num_paired_similar
                        ...
                        # # If first move was optimal
                        if first_single_move_optimal:

                            tmp_opt    += num_first_single + num_first_single_similar
                            if num_first_single > 0:
                                H_opt_single += [get_entropy(T, s1, ja1[0])]*num_first_single
                            if num_first_single_similar > 0:
                                H_opt_single += [get_entropy(T, s1, ja1[1])]*num_first_single_similar
                                
                        else:
                            
                            tmp_subopt += num_first_single + num_first_single_similar
                            if num_first_single > 0:
                                H_subopt_single += [get_entropy(T, s1, ja1[0])]*num_first_single
                            if num_first_single_similar > 0:
                                H_subopt_single += [get_entropy(T, s1, ja1[1])]*num_first_single_similar
                        
                        # If second move was optimal
                        if second_single_move_optimal:

                            tmp_opt    += num_second_single + num_second_single_similar
                            if num_second_single > 0:
                                H_opt_single += [get_entropy(T, s2, ja2[0])]*num_second_single
                            if num_second_single_similar > 0:
                                H_opt_single += [get_entropy(T, s2, ja2[1])]*num_second_single_similar
                                
                        else:
                            
                            tmp_subopt += num_second_single + num_second_single_similar
                            if num_second_single > 0:
                                H_subopt_single += [get_entropy(T, s2, ja2[0])]*num_second_single
                            if num_second_single_similar > 0:
                                H_subopt_single += [get_entropy(T, s2, ja2[1])]*num_second_single_similar

                    if paired_move_optimal or second_single_move_optimal or first_single_move_optimal:
                        opt += [tmp_opt]
                    if not (paired_move_optimal or second_single_move_optimal or first_single_move_optimal):
                        subopt += [tmp_subopt]
                    ...
                    if replay_backups.shape[0] > 1:
                        ...
                        if single_move_optimal:
                            tmp_opt += num_replays + num_replays_similar
                            if num_replays > 0:
                            ...
                            tmp_subopt += num_replays + num_replays_similar
                            if num_replays > 0:
                        ...
                        for rid in range(replay_backups.shape[0]):
                            ...
                            if sr != -200 and ar != -200:
                                ...
                                if pl == 0:
                                    ...
                                    if cond1 and cond2:
                                        first_single_move_optimal  = ar in get_optimal_move(sr, Q1_true)//4
                                        second_single_move_optimal = ar in get_optimal_move(sr, Q1_true)%4
                                        
                                        if second_single_move_optimal or first_single_move_optimal:
