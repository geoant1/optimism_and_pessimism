...
data_path     = '/u/gantonov/data/subject_data'
sub_data_path = os.path.join(data_path, str(this_sub))

blocks_max_rwd = np.load(os.path.join(sub_data_path, 'blocks_max_rwd.npy'), allow_pickle=True)[7:]
blocks_rwd     = np.load(os.path.join(sub_data_path, 'blocks_obt_rwd.npy'), allow_pickle=True)[7:]
...
def main(params):
    ...
    bl = 0
    these_states = sts[bl]
    m = a.explore_one_move(world1, idcs1, states=these_states, num_trials=6)

    for bl in range(1, 6):
        these_states = sts[bl]
        m = a.explore_one_move(world1, idcs1, states=these_states, num_trials=12)

    bl = 6
    these_states = sts[bl]
    m = a.explore_one_move(world1, idcs1, states=these_states, num_trials=48)
...
#s  = this_sub + 1
save_path = '/u/gantonov/out_new/save_params_%u'%this_sub
...
def simulation(params):
    ...
    r_data = parallel.sim_pool.map(main, [params]*10)
    return r_data
...
num_abc = 501
eps   = np.logspace(np.log10(0.6), np.log10(0.10), 55)
sigma = np.logspace(np.log10(0.5), np.log10(0.01), 55)

save_folder = '/u/gantonov/out_new/save_params_%u'%this_sub
