...
def sample_params(t_mean, covar, num):
    params = []
    for i in range(num):
        while True:
            these_params = np.atleast_1d(scipy.stats.multivariate_normal.rvs(mean=t_mean,cov=covar,size=1))
            cond1 = these_params[3] > 1 or these_params[3] < 0
            cond2 = these_params[4] > 1 or these_params[4] < 0
            cond3 = these_params[6] > 1 or these_params[6] < 0
            cond4 = these_params[7] > 1 or these_params[7] < 0
            cond5 = these_params[8] > 1 or these_params[8] < 0
            cond6 = these_params[9] > 1 or these_params[9] < 0
            cond7 = these_params[12] > 1 or these_params[12] < 0
            cond8 = these_params[13] > 1 or these_params[13] < 0
            cond9 = these_params[14] > 1 or these_params[14] < 0
            cond10 = these_params[0] <= 0
            cond11 = these_params[1] <= 0
            cond12 = these_params[2] <= 0
            cond13 = these_params[15] < 0
            if cond1 or cond2 or cond3 or cond4 or cond5 or cond6 or cond7 or cond8 or cond9 or cond10 or cond11 or cond12 or cond13:
                pass
            else:
                break
        params.append(tuple(these_params))
    if num == 1:
        return these_params
    else:
        return params
...
def step(info_in,sim_pool=None):
    ...
    while True:
        if t ==0: #draw from prior
            if abcsampler.mpi_splitcomm:
                trial_t = [tuple(call_prior() for call_prior in abcsampler.prior) for i in range(sim_pool.size)]
                x1 = abcsampler.model(trial_t, sim_pool)
                x2 = abcsampler.model(trial_t, sim_pool)
                x3 = abcsampler.model(trial_t, sim_pool)
                rho1 = abcsampler.dist(x1)
                rho2 = abcsampler.dist(x2)
                rho3 = abcsampler.dist(x3)
                rho  = np.mean(np.vstack((rho1, rho2, rho3)), axis=0)
            else:
            ...
            if abcsampler.mpi_splitcomm:
                # Check that the newly sampled parameters are within the range
                # trial_t = [tuple(np.atleast_1d(scipy.stats.multivariate_normal.rvs(mean= t_old,cov=covariance,size=1))) for i in range(sim_pool.size)]
                trial_t = sample_params(t_old, covariance, sim_pool.size)
                x1 = abcsampler.model(trial_t, sim_pool)
                x2 = abcsampler.model(trial_t, sim_pool)
                x3 = abcsampler.model(trial_t, sim_pool)
                rho1 = abcsampler.dist(x1)
...
class ABC_class(object):
        ...
        def sample_loop(self,t):
                ...
                if self.mpi or self.mp:
                        if self.mp:
                                pool_outputs = self.parallel.pool.map(self.classstep, list(zip([t]*(self.npart),range(self.npart))))
                        else:
                                pool_outputs = self.parallel.pool.map(wrapper_func, list(zip([t]*(self.npart),\
                        ...
                        if t:
                                ...
                                for part in range(self.npart):
                                        tmp.append(self.particle_weight([t, part]))
                                self.wgt[t] = tmp
                                #self.wgt[t] = self.parallel.pool.map(self.particle_weight, list(zip([t]*(self.npart),range(self.npart))))
                else:
            ...
            while True:
                if t ==0: #draw from prior
                    ...
                    if self.variance_method == 4:
                        ...
                        covariance = self.variance
                    while True:
                        these_params = np.atleast_1d(scipy.stats.multivariate_normal.rvs(mean=t_old,cov=covariance,size=1))
                        cond1 = these_params[3] > 1 or these_params[3] < 0
                        cond2 = these_params[4] > 1 or these_params[4] < 0
                        cond3 = these_params[6] > 1 or these_params[6] < 0
                        cond4 = these_params[7] > 1 or these_params[7] < 0
                        ...
                        cond7 = these_params[12] > 1 or these_params[12] < 0
                        cond8 = these_params[13] > 1 or these_params[13] < 0
                        cond9 = these_params[14] <= 0
                        cond10 = these_params[0] <= 0
                        cond11 = these_params[1] <= 0
                        cond12 = these_params[2] <= 0
                        cond13 = these_params[15] > 1 or these_params[15] < 0
                        if cond1 or cond2 or cond3 or cond4 or cond5 or cond6 or cond7 or cond8 or cond9 or cond10 or cond11 or cond12 or cond13:
                            pass
                        else:
                            break

                    trial_t = these_params
